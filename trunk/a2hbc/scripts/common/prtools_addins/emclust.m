...
%                     end
  		...
  		if (islabtype(a,'soft'))
				...
				new_lab = d*labeld;
                cs_new_lab = classsizes(dataset(d,new_lab));
  ...
  if (islabtype(a,'soft'))
  	...
  	while (change > epsilon)       	% EM loop, run until labeling is stable.
        ...
        while (change ~= 0 && any(lab ~= new_lab)) % EM loop, run until labeling is stable.
            ...
            std_a = std(+a);
            w_em = (a .* (ones(size(+a))+ bsxfun(@times, max( [repmat(0.001,size(+a)); 0.01*std_a] ), randn(size(+a))))) * w_clust;             % 1. Compute classifier, crisp output.
