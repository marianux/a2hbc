...
if isfield(heasig,'freq')  % if exists sampling frequency field
    ...
    if isfield(heasig,'nsamp')  % if exists number of samples per signal
        if ~isempty(deblank(s))&&~isempty(findstr(s,':'))
            [s1 s] = strtok(s,':');
            hour = strtrim(s1);
            [s1 s] = strtok(s,':');
            min = strtrim(s1);
            ...
            if ~isempty(deblank(s))&&~isempty(findstr(s,'/'))
                ...
                s1 = strtok(s,pp);
                year = strtrim(s1);
                [yy a] = datevec(['01/01/' year]);
                heasig.bdate = [day '/' month '/' num2str(yy)];
            end
        else
            hour = '0';
            min = '0';
...
%     heasig = hsig;        
    ...
    for i = 1:heasig.nsig
        ...
        % format
        [s3,s4]=strtok(s);
        a=[findstr(s3,'x') findstr(s3,':') findstr(s3,'+')];
        if isempty(a)
