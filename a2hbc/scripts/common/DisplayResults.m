function [dsResult ConfusionMat lablist global_performances ] = DisplayResults( varargin )
    ...
    p.addParamValue('dsResult', [], @(x)( isnumeric(x) | isa(x,'dataset') ) );
    ...
    p.addParamValue('SupportDataset', [], @(x)( isa(x,'dataset') ));
    p.addParamValue('TrainedMapping', [], @(x)( isa(x,'mapping') && istrained(x)) );
