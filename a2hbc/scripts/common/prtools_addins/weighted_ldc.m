...
function [W,r,s,dim] = weighted_ldc(a, r, s, dim, bUseWeight_Vector)
	...
	if (nargin < 1) | (isempty(a))  % No input arguments: 
		W = mapping(mfilename,{r,s,dim,bUseWeight_Vector}); % return an untrained mapping.
	...
	elseif any(isnan([r,s,dim]))    % optimize regularisation parameters
        ...
        if( ~isempty(OtherFeatures) )
            U(OtherFeatures,:)=X(OtherFeatures,:)*weight_matrix'/K;
        end
        
        if( ~isempty(DirectionalFeatures) )
            U(DirectionalFeatures,:)=angle(exp(1i*X(DirectionalFeatures,:))*weight_matrix'/K);
        end
...
%         end
        ...
        weight_matrix_sqrt = sqrt(weight_matrix);

        if( ~isempty(OtherFeatures) )
            iAux(OtherFeatures,:) = (  ( X(OtherFeatures,:).*(ones(length(OtherFeatures),c)*weight_matrix_sqrt) ) - U(OtherFeatures,:)*weight_matrix_sqrt );
        end
